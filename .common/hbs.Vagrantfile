...
cpus = 2
description = "{{template_json.variables.description}}"
memory = 4096
...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "{{template_json.variables.latestos_tag}}" do |os|
    os.vm.box="{{template_json.variables.vagrantup_user}}/{{template_json.variables.box_basename}}"
    os.vm.hostname = "{{hostname}}"
...
{{#each template_json.builders }}
{{#if (eq this.type "hyperv-iso")}}
...
{{/if}}
{{#if (eq this.type "virtualbox-iso")}}
